...
% For FMCW RADAR, the sweep time should be at least five to six times 
% the round trip time.
rttScale = 5.5;
%% User Defined Range and Velocity of target
R = 110;  % Initial position
v = -20;  % Initial velocity that remains constant.
...
for i=1:length(t)    
    ...
    Rx(i) = cos(2 * pi * (fc * (t(i) - tau) + alpha * (t(i) - tau)^2 / 2));
...
% map. Implement CFAR on the generated RDM
...
Gr = 8;  % Guard cells (range dimension)
...
TotalTrainingCells = (2*Tr + 2*Gr + 1) * (2*Td + 2*Gd + 1) - (2*Gr + 1) * (2*Gd + 1);
...
for i = 1:(Nr/2-(2*Gr+2*Tr+1))
    for j = 1:(Nd-(2*Gd+2*Td+1))
        ...
        threshold = pow2db(sum_train/TotalTrainingCells)*offset;
